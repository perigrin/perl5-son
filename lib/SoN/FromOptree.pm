...
class SoN::FromOptree 0.01 {
    ...
    sub _build_single_exit ($factory, $exits) {
        if (@$exits == 1) {
            ...
            # Produce-time control (i3): control is carried on control_in,
            # never flattened into inputs. This subsumes the old ctrl-is-a-
            # stmt-effect-Call special case (previously a VOID stmt-effect
            # Call as the trailing control could not lead inputs without
            # being misread as the return VALUE, so it was put second
            # instead) -- control_in is never a data input, so a Return's
            # inputs is always exactly [value] regardless of what kind of
            # node control is.
    ...
    sub _handle_entersub ($cv, $op, $sim, $factory, $ctx) {
        ...
        if (defined $ctx->{pending_method}) {
            ...
            # i3) so it is ordered and survives DCE, and do not push a value.
        ...
        # the control chain via control_in (produce-time control, i3) so it
        # is ordered and survives DCE, exactly as the void METHOD branch does
    ...
    sub _step ($cv, $op, $sim, $factory, $opmap, $ctx) {
        ...
        if ($name =~ /^(i_)?(pre|post)(inc|dec)$/) {
            ...
            if (defined $lvalue) {
                ...
                # Control is carried on control_in (produce-time control, i3).
        ...
        if ($name eq 'sassign') {
            ...
            # control chain via control_in (produce-time control, i3) so it is
        ...
        if ($name eq 'print') {
            ...
            # control_in (produce-time control, i3) so the stdout effect is
        ...
        if ($opmap->is_known($name) && !$opmap->is_branch($name) && !$opmap->is_loop($name)) {
            ...
            if (defined $node_type) {
                ...
                # control, i3), and the Call becomes the new memory version so
                # a later whole-array read (Length/element) observes the
                # drained array. Stamp with the array's element type so the
                # removed value (and anything derived from it) carries a repr.
                ...
                # i3) -- mirroring the void entersub path, so the effect is
    ...
    sub _translate_foreach_range ($cv, $enteriter, $sim, $factory, $opmap, $visited, $low, $high) {
        ...
        # backend recovers this continuation test unambiguously. Produce-time
        # control (i3): set directly on control_in, folding the transitional
        # loop_control side-table entry into the real edge.
    ...
    sub _translate_foreach_array ($cv, $enteriter, $sim, $factory, $opmap, $visited, $array) {
        ...
        my $range_cond = $factory->make('NumGt',
            inputs => [$len, $i_phi],
            stamp  => SoN::IR::Stamp->new(type => 'Boolean'));
        # Produce-time control (i3): set directly on control_in.
        $range_cond->set_control_in($loop_node);
